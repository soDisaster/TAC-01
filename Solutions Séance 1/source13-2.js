...
table = {
	creer : function(tableauValeursClasses, classeCssTableau) {
		...
		tableau.addEventListener("click",this.clickedFunction);
		for ( var indiceLigne = 0; indiceLigne < tableauValeursClasses.length; indiceLigne++) {
			...
			for ( var indiceColonne = 0; indiceColonne < ligne.length; indiceColonne++) {
				...
				htmlCellule.texte = htmlTexte;
				htmlCellule.indiceColonne = indiceColonne;
				htmlCellule.indiceLigne = indiceLigne;
			}
		}
		return tableau;
	},
	clickedFunction : function(event) {
		var nouveauTexte=prompt("Nouveau texte pour la cellule "+event.target.indiceColonne+","+event.target.indiceLigne,event.target.texte.data);
		event.target.texte.data=nouveauTexte;
	}
};
